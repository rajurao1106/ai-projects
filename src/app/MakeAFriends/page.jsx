...
const AIChat = () => {
  ...
  const startListening = useCallback(() => {
    if (recognition) {
      recognition.stop(); // Stop any previous recognition to prevent conflicts
    }

    recognition = new (window.SpeechRecognition || window.webkitSpeechRecognition)();
    recognition.lang = "en-US";
    recognition.continuous = true; // Keeps listening after each speech
    ...
    recognition.onend = () => {
      setListening(false);
      if (!processing) startListening(); // Restart listening after response
    };
  ...
  const speak = (text) => {
    ...
    speech.lang = "en-US";
    speech.onend = () => startListening(); // Restart listening after speaking
  ...
  useEffect(() => {
    ...
    return () => {
      if (recognition) recognition.stop(); // Clean up when unmounting
    };
  }, []);

  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-6">
      <h1 className="text-3xl font-bold mb-6">🤖 AI Friend (Continuous)</h1>
      <button
