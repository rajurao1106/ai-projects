...
const AIChat = () => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (SpeechRecognition) {
        ...
        recog.onend = () => {
          setListening(false);
          if (!processing) {
            setTimeout(() => recog.start(), 1000); // 🔄 Auto-restart mic
          }
        };
  ...
  const speak = (text) => {
    ...
    speech.pitch = 1.1;
    speech.onend = () => {
      if (recognition) recognition.start(); // 🔄 Auto-start mic after AI response
    };
  ...
  useEffect(() => {
    if (recognition && !listening) {
      recognition.start(); // 🔄 Auto-start mic on load
    }
  }, [recognition]);
      ...
      <div className="w-full lg:w-1/2 h-screen flex flex-col items-center justify-between p-8 lg:p-12 relative">
        ...
        <div className="flex-1 flex flex-col items-center justify-center w-full max-w-md z-10">
          <motion.div
            ...
            className="relative bg-indigo-600 text-white px-10 py-5 rounded-full shadow-lg transition-all duration-300 flex items-center gap-4 text-lg font-medium"
          >
            <span className="text-2xl animate-pulse">🎤</span>
            <span>{listening ? "Listening..." : "Speak Anytime"}</span>
          </motion.div>
        </div>
      </div>
    </div>
